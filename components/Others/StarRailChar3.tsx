...
import { getPostsHomeFeed } from "@/services/post/post";
...
const StarRailChar2 = () => {
  ...
  const fetchChars = async () => {
    try {
      ...
      const _response = await getPostsHomeFeed(session.userToken.accessToken);
      if (_response) {
       const newData = _response.data.posts.data
        .map((item: any): PostProps => ({
          id: item.userId,
          userId: item.userId,
          content: item.content,
          images: item.postImages.map((image: any) => image.url),
          createdAt: item.createdAt.split("T")[0],
          isLiked: item.emotionByMe ?? false,
          liked: item.likeCount, // Initialize liked property
          // location: "Đà Nẵng",
        }))
        // .slice(2);
      // setChars(response.data);
        ...
        <View style={styles.itemOuterContainer}>
          <View style={styles.itemContainer}>
            <Pressable onPress={() => router.push(`/post/${item.id}`)}>
              ...
              <Image source={require("../../assets/images/tripfeed/da-nang.jpg")} style = {styles.avatar} />
            </Pressable>
          ...
          <View style={styles.textContainer}>
            ...
            <ReadMoreText text={item.content} numberOfLines={3} />
            {/* <ReadMoreText
  ...
  return (
    <View style={styles.container}>
      <FlashList
        ...
        renderItem={({item}: {item: PostProps}) => <RenderItem item={item} />}
