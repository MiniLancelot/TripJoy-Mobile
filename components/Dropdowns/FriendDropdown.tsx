...
import { get_friends } from "@/services/user/friend_request";
...
interface ProvinceDropdownProps {
  _value: FriendProps;
  setValue: (value: FriendProps) => void;
  bearer: string;
  placeholder?: string;
}
...
const FriendDropdown = ({
  ...
  const fetchMoreData = async () => {
    ...
    setLoading(true);
    try {
      const response = await get_friends(bearer, {
        pageIndex: page,
        pageSize: 10,
      });

      // if (!response?.data?.user) throw new Error("Invalid data structure");

      const newData = response.data.users.data.map((item: any) => ({
        label: item.userName,
        value: item.id,
      }));
  ...
  const searchProvince = async (searchText: string) => {
    ...
    try {
      ...
      if (response?.data?.users) {
        const searchedData = response.data.users.data.map((item: any) => ({
          label: item.userName,
          value: item.id,
        }));
  ...
  const loadMoreItems = () => {
    if (!loading && !allItemsLoaded) setPage((prevPage) => prevPage + 1);
  };
