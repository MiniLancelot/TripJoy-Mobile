...
import { addPlan, getPlanLocationsByPlanId, updatePlan } from "@/services/plan/plan";
...
const UpdatePlan = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await getPlanLocationsByPlanId(session.userToken.accessToken, sharedId);
      if (response) {
        console.log(response.data.plan);
        const plan = response.data.plan;
        setData ({
          title: plan.title,
          startDate: plan.estimatedStartDate.split("T")[0],
          endDate: plan.estimatedEndDate.split("T")[0],
          estimatedBudget: plan.estimatedBudget,
          provinceStart: {provinceId: plan.provinceStart.provinceId, provinceName: plan.provinceStart.provinceName},
          provinceEnd: {provinceId: plan.provinceEnd.provinceId, provinceName: plan.provinceEnd.provinceName},
          method: plan.method,
  ...
  const _updatePlan = async () => {
    try {
      ...
      const response = await updatePlan(_form, sharedId,session.userToken.accessToken);
      if (response) {
      ...
      Toast.show({
        type: "success",
        text1: "Cập nhật chuyến đi thành công",
